...
- Optional `TimeZone=` in the legacy/PUSH-SDK handshake so an ADMS-managed device
  holds the right wall clock instead of drifting to the response's GMT `Date`
  header. Set `zkteco.adms.timezone_offset_minutes` (env
  `ZKTECO_ADMS_TIMEZONE_OFFSET_MINUTES`) to the device offset east of UTC in
  minutes (e.g. `180` for UTC+3); left unset the line is omitted and the device
  keeps its own timezone, so existing deployments are unchanged.
...
[Unreleased]: https://github.com/msaied/zkteco-php/compare/v0.2.2...HEAD
[0.2.2]: https://github.com/msaied/zkteco-php/compare/v0.2.1...v0.2.2
